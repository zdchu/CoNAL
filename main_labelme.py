...
train_dataset = Dataset(mode='train', dataset=dataset, sparsity=0, test_ratio=0.1)
...
def main(opt, model=None):
    ...
    for epoch in range(opt.num_epochs):
        ...
        cls_out, output = model(train_dataset.X_val.cuda(), mode='train')
        pred, output = output.softmax(1).max(1)
        likelihood = -torch.mean(torch.log(pred[train_dataset.ans_val != -1])).item()
        # IPython.embed()
        print(likelihood)
